...
def main():
    ...
    # Sidebar - dùng reset_counter để tạo key duy nhất
    with st.sidebar:
        st.markdown("## 🎮 MENU")
        if st.button("🆕 BẮT ĐẦU MỚI", key=f"new_game_{st.session_state.reset_counter}"):
            st.session_state.game = CombatGame()
            st.session_state.game_active = False
            st.session_state.show_leaderboard = False
    ...
    if st.session_state.show_leaderboard:
        ...
        if top:
            for i, s in enumerate(top):
                medal = ["🥇", "🥈", "🥉"][i] if i < 3 else f"#{i+1}"
                st.markdown(f"{medal} **{s['name']}** - {s['score']} điểm - 📅 {s['date']}")
        if st.button("🔙 QUAY LẠI GAME", key=f"back_{st.session_state.reset_counter}"):
            st.session_state.show_leaderboard = False
            st.rerun()
        return
    ...
    if not st.session_state.game_active and not game.game_active:
        ...
        name = st.text_input("👤 Nhập tên chiến binh:", max_chars=20, key=f"player_name_{st.session_state.reset_counter}")
        if st.button("🚀 BẮT ĐẦU CHIẾN ĐẤU", key=f"start_{st.session_state.reset_counter}", use_container_width=True):
    ...
    if game.player_hp <= 0:
        ...
        with col2:
            if st.button("🔄 CHƠI LẠI", key=f"play_again_over_{st.session_state.reset_counter}", use_container_width=True):
                st.session_state.game = CombatGame()
                st.session_state.game_active = False
                st.session_state.reset_counter += 1
                st.rerun()
    ...
    if game.victory:
        ...
        col1, col2 = st.columns(2)
        with col1:
            if st.button("🔄 CHƠI LẠI", key=f"play_again_victory_{st.session_state.reset_counter}", use_container_width=True):
                st.session_state.game = CombatGame()
                st.session_state.game_active = False
                st.session_state.reset_counter += 1
                st.rerun()
        with col2:
            if st.button("🏆 BẢNG XẾP HẠNG", key=f"view_rank_{st.session_state.reset_counter}", use_container_width=True):
    ...
    # Nút hành động - dùng reset_counter để tạo key duy nhất mỗi lần reset
    col1, col2, col3 = st.columns(3)
    
    with col1:
        if st.button("⚔️ TẤN CÔNG ⚔️", key=f"attack_{st.session_state.reset_counter}", use_container_width=True):
    ...
    with col2:
        if st.button("💊 HỒI MÁU (20đ) 💊", key=f"heal_{st.session_state.reset_counter}", use_container_width=True):
    ...
    with col3:
        if st.button("🛡️ PHÒNG THỦ 🛡️", key=f"defend_{st.session_state.reset_counter}", use_container_width=True):
